...
// VideoEncoder and the mp4-muxer audio/video tracks. No fMP4 header is produced
// here (the init segment is generated by mp4-muxer in JS).
...
public:
  ...
  // In this mode WASM does NOT mux: it demuxes, decodes video to I420, and forwards audio
  // packets (already AAC, or AAC produced by the existing EAC3->AAC transcode) to JS, which
  // muxes everything with mp4-muxer.
  ...
  // Audio units (already-encoded AAC bytes + timestamps in seconds) waiting to be emitted.
  std::deque<AudioUnit> pending_audio;
  // When true, the AAC encoder output is captured into pending_audio instead of being muxed
  // (used for EAC3/AC3 -> AAC in transcode mode, where there is no output muxer).
  ...
  int send_audio_frame_to_encoder(AVFrame *frame, AVStream *out_stream) {
    ...
    while (response >= 0) {
      ...
      // Transcode mode: capture the AAC bytes for JS (Mediabunny) instead of muxing here.
      if (capture_audio) {
        AudioUnit unit;
